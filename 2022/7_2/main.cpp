...
struct dir{
	...
	long int sol_b(int needed_space){
		dir* solve = this;
		for(auto x : childs){
			if( x != nullptr){
				if( (x->total_size >= needed_space) && (x->sol_b(needed_space) < solve->total_size)){
					solve = x;
				}
			}
		}
		return solve->total_size;
	}
...
int main(int argc, char** argv){
	...
	int needed_space =30000000 - (70000000 - 43313415); 
	
	//	solution = head.print_sol(0); 
	//	cout << "Lösung: " << solution << endl;
	//	head.print_dir(0);
	cout << "Lösung: " << head.sol_b(needed_space) << endl;
	return 0;
}
